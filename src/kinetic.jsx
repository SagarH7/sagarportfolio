    ...
    <section style={{ padding: "120px 48px", maxWidth: 1400, margin: "0 auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 1, background: "var(--line)", border: "1px solid var(--line)" }}>
        ...
        <Reveal key={i} delay={i * 0.06}>
            <div style={{ padding: "28px 24px", background: "var(--bg)", height: "100%" }}>
              <div style={{ fontFamily: "var(--serif)", fontSize: "clamp(28px, 3vw, 44px)", lineHeight: 1.1, letterSpacing: "-0.03em", color: "var(--ink)", marginBottom: 12, minHeight: "auto" }}>
                <CountUp value={m.value} />
              </div>
              <div style={{ fontWeight: 600, fontSize: 15, marginBottom: 6 }}>{m.label}</div>
              <div style={{ fontSize: 13, color: "var(--ink-3)", lineHeight: 1.5 }}>{m.sub}</div>
            </div>
          </Reveal>
        )}
      </div>
